...
public class DefeatBossesGameRules : GameRules
{
    ...
    private DefeatBossesGameOverCondition _defeatBossesCondition;
    ...
    protected override void Awake()
    {
        ...
        if (_leaderboard == null)
            _leaderboard = gameObject.AddComponent<DefeatBossesLeaderboard>();

        if (_gameOverCondition == null)
            _gameOverCondition = gameObject.AddComponent<DefeatBossesGameOverCondition>();

        // Cache reference to specific game over condition type
        _defeatBossesCondition = _gameOverCondition as DefeatBossesGameOverCondition;

        if (_leaderboard == null || _gameOverCondition == null)
            Debug.LogError("DefeatBossesGameRules requires Leaderboard and GameOverCondition components!");
    }
    ...
    protected override IEnumerator GameOverSequence()
    {
        ...
        Debug.Log("Victory! All bosses defeated!");
    ...
    public int GetRemainingBossCount() => _defeatBossesCondition?.GetRemainingBossCount() ?? 0;
    public float GetBossDefeatProgress() => _defeatBossesCondition?.GetBossDefeatProgress() ?? 0f;
